(()=>{
  ...
  const write=(rows)=>{try{localStorage.setItem(STORE,JSON.stringify(rows.slice(-1500)))}catch(e){}};
  function track(event_name,data={}){
    const rows=read();
    rows.push({id:safeUUID(),created_at:now(),visitor_id:visitorId,session_id:sessionId,event_name,event_value:data.event_value??null,section:data.section??null,path:location.pathname+location.search,meta:{...baseMeta,...(data.meta||{})}});
    write(rows);
    try{window.dispatchEvent(new CustomEvent('tdp-analytics',{detail:rows.at(-1)}))}catch(e){}
  }
  window.tdpTrack=track;

  const first=!localStorage.tdpSeenBefore;
  track('site_opened',{meta:{first_visit:first}});
  localStorage.tdpSeenBefore='1';

  const scrollMarks=new Set();
  addEventListener('scroll',()=>{
    const max=document.documentElement.scrollHeight-innerHeight;if(max<=0)return;
    const pct=Math.round(scrollY/max*100);
    [25,50,75,100].forEach(n=>{if(pct>=n&&!scrollMarks.has(n)){scrollMarks.add(n);track('scroll_depth',{event_value:String(n)})}});
  },{passive:true});
  ...
  document.addEventListener('click',e=>{
    ...
    const text=(el.getAttribute('aria-label')||el.textContent||el.id||'control').trim().replace(/\s+/g,' ').slice(0,80);
  ...
  const io='IntersectionObserver'in window?new IntersectionObserver(entries=>entries.forEach(entry=>{
    ...
    if(entry.isIntersecting&&entry.intersectionRatio>=.35){
      if(!active.has(section)){active.set(section,performance.now());track('section_enter',{section})}
    }else if(active.has(section)){
      const ms=Math.round(performance.now()-active.get(section));active.delete(section);track('section_exit',{section,meta:{dwell_ms:ms}})
    }
  ...
  new MutationObserver(ms=>ms.forEach(m=>{
    ...
    const old=classSeen.get(el)||'';const cur=typeof el.className==='string'?el.className:'';classSeen.set(el,cur);
    ...
    if(el.id==='envelopeCard'){
      if(!old.includes('is-unlocking')&&cur.includes('is-unlocking'))track('password_correct');
      if(!old.includes('wrong-shake')&&cur.includes('wrong-shake'))track('password_wrong');
      if(!old.includes('password-visible')&&cur.includes('password-visible'))track('password_panel_opened');
    }
